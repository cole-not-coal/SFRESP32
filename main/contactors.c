...
void turn_on_contactor(byte byNContactorID, byte byNContactorGPIOPin) {
    ...
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL, DUTYCYCLE_CONTACOTR_ON); // Set duty cycle to 100
    ledc_update_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL); // Update duty cycle
...
void hold_contactor_on(byte byNContactorID, byte byNContactorGPIOPin) {
    ...
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL, DUTYCYCLE_CONTACOTR_HOLD); // Set duty cycle to 80
    ledc_update_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL); // Update duty cycle
...
void turn_off_contactor(byte byNContactorID, byte byNContactorGPIOPin) {
    ...
    ledc_set_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL, DUTYCYCLE_CONTACOTR_OFF); // Set duty cycle to 0
    ledc_update_duty(LEDC_HIGH_SPEED_MODE, CONTACTOR_PWM_CHANNEL); // Update duty cycle
